...
func _ready():
	...
	# イラストレーター非表示
	self.monkey().of_director().illustrator().hide()
...
func on_exit():
	...
	# GUI - メッセージ・ウィンドウ
	self.monkey().of_director().programmer().owner_node().images.find_node("■上_大").hide()
	self.monkey().of_director().programmer().owner_node().images.find_node("■下").hide()
	# テロップ非表示
	self.monkey().get_my_telop("TextBlock").text = ""
	self.monkey().get_my_telop_canvas_layer().hide()

	# BGM 停止	
	self.monkey().of_director().programmer().owner_node().bg_musics.find_node("🎵キーコンフィグ").stop()
...
func set_key_accepted():
	self.monkey().of_director().programmer().owner_node().sound_fx.find_node("🔔キーコンフィグ受入音").play()


# キーコンフィグ　ボタン設定が拒否
func set_key_denied(reason):
	self.monkey().of_director().programmer().owner_node().sound_fx.find_node("🔔キーコンフィグ不可音").play()
...
func set_key_canceled():
	self.monkey().of_director().programmer().owner_node().sound_fx.find_node("🔔キーコンフィグ取消音").play()
	self.monkey().get_my_telop("TextBlock").text = ""
...
func set_press_message_to_button(step):
	...
	# 完了時
		...
		self.monkey().of_director().programmer().owner_node().sound_fx.find_node("🔔キーコンフィグ完了音").play()
...
func on_process(delta):
	...
	if self.current_step == 0:
		self.monkey().of_director().programmer().owner_node().bg_musics.find_node("🎵キーコンフィグ").play()
		self.current_step += 1
		self.clear_count()
