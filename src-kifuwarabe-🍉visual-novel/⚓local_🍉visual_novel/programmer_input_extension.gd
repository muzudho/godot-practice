...
func parse_virtual_key_on_process_if_it_change(vk_name):
	...
	if is_changed:
		var vk_plan_state = self.owner_node().get_plan_key_state(vk_name)
		...
		print("［入力拡張］　［" + vk_name + "］キーは状態変化した。 vk_plan_state:" + str(vk_plan_state) + " vk_accepted_state:" + str(vk_accepted_state) + " vk_occurence:" + vk_occurence + " vk_during:" + vk_during)

		# 仮想キーを押下したという建付け
		self.monkey().of_staff().programmer().scenario_player().input_node().on_virtual_key_input(
				vk_name,
				vk_plan_state, #vk_accepted_state,
				vk_occurence,
				vk_during)
