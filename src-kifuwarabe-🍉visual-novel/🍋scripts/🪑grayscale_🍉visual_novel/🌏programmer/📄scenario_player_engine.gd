...
func get_current_section_size_of_scenario():
	...
	var section_array = self.hub().get_director_hub().scenario_writers_hub().get_section_array(scenario_node_name, section_name)
...
func get_current_paragraph_of_scenario():
	...
	var merged_scenario_document = self.hub().get_director_hub().scenario_writers_hub().get_merged_scenario_document(department_value.name)
...
func on_choice_selected(row_number):
	...
	# 辞書
	var choices_mappings_a = self.hub().get_director_hub().scenario_writers_hub().get_merged_choices_mappings(department_name)
