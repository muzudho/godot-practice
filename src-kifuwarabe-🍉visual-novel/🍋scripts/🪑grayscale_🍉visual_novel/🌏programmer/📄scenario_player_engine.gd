...
func get_current_department_value():
	return self.monkey().owner_node().get_department_value(
...
func on_process(delta):
	...
	if 0.0 < self.monkey().of_director().owner_node().sleep_seconds:
		self.monkey().of_director().owner_node().sleep_seconds -= delta
