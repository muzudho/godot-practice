...
func appear():
	...
	if not self.get_message_window("中央").visible:
		self.get_message_window("中央").set_visible_subtree(true)

	#	再開
	if not self.get_message_window("中央").is_processing():
