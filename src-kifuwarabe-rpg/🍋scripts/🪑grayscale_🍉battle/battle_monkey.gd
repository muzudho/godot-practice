...
func refresh_sente_solidity_of_castle():
	monkey().of_staff().programmer().owner_node().telops.find_node("Ｔ戦闘シーン/城の堅さ_下").text = StringHelper.number_to_zenkaku_text(self.get_game_sheet_for_battle().solidity_of_castle[0], 4)
