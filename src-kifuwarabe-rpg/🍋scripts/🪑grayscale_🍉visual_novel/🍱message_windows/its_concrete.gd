...
func on_talked_2():
	...
	var illust_a = self.monkey().of_director().programmer_monkey().images.find_node(self.name)
...
func on_page_forward():
	...
	if self.is_choices():
		...
		# カーソル音
		self.monkey().of_director().programmer_monkey().get_instruction(&"📗SoundFx").play_se("🔔選択肢確定音")

		var row_number = self.get_row_number_of_choices()
		print("［伝言窓　”" + self.name + "”］　選んだ選択肢行番号：［" + str(row_number) + "］")

		# 選択肢の行番号を、上位ノードへエスカレーションします
		self.monkey().of_director().programmer_monkey().scenario_player().on_choice_selected(row_number)
		...
		# 効果音
		self.monkey().of_director().programmer_monkey().get_instruction(&"📗SoundFx").play_se("🔔ページめくり音")
